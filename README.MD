...
- **장점**: 트랜잭션 관리가 상대적으로 쉬움
- **단점**: 서비스 간 의존성이 강하며, 호출하는 서비스가 느려지면 전체 시스템이 지연됨 (강한 결합)
...
- **사용 사례**:
    ...
    - 대량의 트래픽 처리가 필요하거나, 서비스 간 결합도를 낮추고 싶을 때
- **장점**: 서비스 간 의존성이 낮고, 응답성이 좋으며, 시스템 가용성이 높음 (느슨한 결합)
- **단점**: 결과적 일관성(Eventual Consistency)에 대한 고려가 필요함
- **구현 방식**: Message Broker (Kafka, RabbitMQ 등)
- **핵심 패턴**: **Choreography SAGA**, **Data Replication**, **Eventual Consistency**
